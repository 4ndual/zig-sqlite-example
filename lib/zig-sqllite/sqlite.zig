...
test "fuzzing" {
    // Add logging imports if not already present at the top of your file
    const log = std.log;

    const global = struct {
        fn testOne(input: []const u8) anyerror!void {
            log.info("Starting test iteration with input length: {}", .{input.len});
            ...
            db.execDynamic(input, .{}, .{}) catch |err| switch (err) {
                error.SQLiteError => {
                    log.info("SQLite error occurred during dynamic execution: {}", .{err});
                    return;
                },
                error.ExecReturnedData => {
                    log.info("Execution returned data unexpectedly", .{});
                    return;
                },
                error.EmptyQuery => {
                    log.info("Empty query detected", .{});
                    return;
                },
                else => {
            ...
            log.debug("Dynamic execution completed", .{});
            ...
            db.execDynamic(
                ...
                error.SQLiteError => {
                    log.info("SQLite error during insert: {}", .{err});
                    return;
                },
                else => {
                    log.err("Unexpected error during insert: {}", .{err});
                    return err;
                },
            };
            ...
            var stmt = db.prepareDynamic("SELECT name, data FROM test") catch |err| switch (err) {
                error.SQLiteError => {
                    log.info("SQLite error during prepare: {}", .{err});
                    return;
                },
                else => {
                    log.err("Unexpected error during prepare: {}", .{err});
                    return err;
                },
            };
            defer stmt.deinit();
            ...
            const row_opt = stmt.oneAlloc(
                ...
                error.SQLiteError => {
                    log.info("SQLite error during row fetch: {}", .{err});
                    return;
                },
                else => {
                    log.err("Unexpected error during row fetch: {}", .{err});
                    return err;
                },
            };

            if (row_opt) |row| {
                log.debug("Row fetched successfully", .{});
                if (!std.mem.eql(u8, row.name.data, "foo")) {
                    log.err("Name field mismatch. Expected 'foo', got '{s}'", .{row.name.data});
                    return error.InvalidNameField;
                }
                if (!std.mem.eql(u8, row.data.data, input)) {
                    log.err("Data field mismatch", .{});
                    return error.InvalidDataField;
                }
                log.debug("Row validation passed", .{});
            } else {
                log.err("No rows found in the result set", .{});
                return error.NoRowsFound;
            }
        }
    };

    log.info("Starting fuzz test with 100 iterations", .{});
    ...
    while (i < 100) : (i += 1) {
        ...
        random.bytes(input_buffer[0..input_len]);

        log.info("Running iteration {}/100 with input length {}", .{ i + 1, input_len });

        // Run the test with random input
        global.testOne(input_buffer[0..input_len]) catch |err| switch (err) {
            // Handle expected errors
            error.SQLiteError, error.ExecReturnedData, error.EmptyQuery, error.InvalidNameField, error.InvalidDataField, error.NoRowsFound => {
                log.debug("Expected error occurred: {}", .{err});
                continue;
            },
            // Propagate unexpected errors
            else => {
                log.err("Unexpected error occurred: {}", .{err});
                return err;
            },
        };
        success_count += 1;
    ...
    log.info("Fuzz test completed. Successful iterations: {}/100", .{success_count});
}
